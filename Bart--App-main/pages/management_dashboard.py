...
def detect_category(sku):
    s = normalize_sku(sku)
    if not s or s == "-" or s == "NONE" or s == "NAN": return "FOOD ITEMS"
...
def make_grid(df, key):
    ...
    if "Total" in df.columns:
        gb.configure_column("Total", type=["numericColumn"], pinned="right", width=100)
    
    AgGrid(df, gridOptions=gb.build(), theme="streamlit", key=key, height=500)
...
if not search_pool.empty:
    search_pool["Search_Label"] = search_pool["SKU"].astype(str) + " | " + search_pool["Item Name"].astype(str) + " (" + search_pool["UOM"].astype(str) + ") [" + search_pool["Schedule"] + "]"
    selected_option = st.selectbox("Type an Item Name, SKU, or UOM...", options=sorted(search_pool["Search_Label"].unique()), index=None, key=f"search_{selected_date_str}")
    if selected_option:
        matched = search_pool[search_pool["Search_Label"] == selected_option]
        result_df = matched[["Item Name", "SKU", "UOM"] + branch_names + ["Total"]]
        make_grid(result_df, "search_grid")
        st.download_button("📥 Export Selected Item", data=to_excel_bytes({selected_option[:20]: result_df}), file_name=f"Report_{selected_date_str}.xlsx")
...
combined_stock = pd.concat([daily_df, weekly_df], ignore_index=True).drop_duplicates(subset=["Item Name", "SKU", "UOM"])
category_dfs = build_category_dfs(combined_stock)
tab_labels = [f"📂 {cat} ({len(sub_df)})" for cat, sub_df in category_dfs.items()]
selected_tab = st.radio("Category Selector", options=tab_labels, index=0, horizontal=True, label_visibility="collapsed")
active_cat = next(cat for cat in category_dfs if f"📂 {cat}" in selected_tab)
make_grid(category_dfs[active_cat], f"grid_{active_cat}")
...
def render(df, title):
    st.subheader(title)
    if not df.empty: make_grid(df, stable_key("grid", title, df))
